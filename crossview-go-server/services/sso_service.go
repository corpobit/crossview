...
func (s SSOService) HandleOIDCCallback(ctx context.Context, code, state string, callbackURL string) (*models.User, error) {
	...
	var roleAttributePathProvided bool
	...
	if oidcConfig.RoleAttributePath != "" {
		...
		roleAttributePathProvided = true
	}
	...
	user, err := s.userRepo.FindOrCreateSSOUser(username, email, firstName, lastName, extractedRole, roleAttributePathProvided)
	if err != nil {
